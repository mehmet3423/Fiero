import { CollectionBasedAffiliateItem } from "@/constants/models/Affiliate";
import { useCart } from "@/hooks/context/useCart";
import { useActiveCollectionCookie } from "@/hooks/useActiveCollectionCookie";
import { toast } from "react-hot-toast";

interface CollectionBasedCollectionProps {
  collectionBasedAffiliateItems: CollectionBasedAffiliateItem[];
}

function CollectionBasedCollection({ collectionBasedAffiliateItems }: CollectionBasedCollectionProps) {
  ...
  const handleAddToCart = async (productId: string) => {
    if (!canAddToCart(collectionBasedAffiliateItems[0].affiliateCollectionId)) {
      toast.error('Farklı bir koleksiyondan ürün ekleyemezsiniz. Lütfen önce mevcut koleksiyonu tamamlayın.');
      return;
    }
    await addToCart(productId);
    if (!activeCollection) {
      createActiveCollection(collectionBasedAffiliateItems[0].affiliateCollectionId, productId);
    }
    else {
  ...
  return (
    <div className="table-responsive">
      <table className="table table-hover">
        ...
        <tbody>
          {collectionBasedAffiliateItems.map((item) => (
...
export default CollectionBasedCollection
