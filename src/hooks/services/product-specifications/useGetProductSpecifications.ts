...
export const useGetProductSpecifications = (productId: string | null) => {
    const { data, isLoading, error, refetch } = useGetData<ProductSpecificationListResponse>({
        url: productId !== null ? `${GET_PRODUCT_SPECIFICATION_LIST}?ProductId=${productId}&_=${Date.now()}` : undefined,
    ...
    return {
        productSpecifications: data?.items.$values,
        isLoading,
        error,
