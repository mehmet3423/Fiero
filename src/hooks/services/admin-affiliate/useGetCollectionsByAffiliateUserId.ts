...
export const useGetCollectionsByAffiliateUserId = (
  userId: string,
  type: AffiliateCollectionType | null
) => {
  const { data, isLoading, error } = useGetData<{
    data: AffiliateCollection[];
  }>({
    url: type
      ? `${GET_AFFILIATE_COLLECTION_BY_AFFILIATE_USER_ID(
        userId
      )}?earningType=${Number(type)}`
      : `${GET_AFFILIATE_COLLECTION_BY_AFFILIATE_USER_ID(userId)}`,
    queryKey: [
      QueryKeys.AFFILIATE_COLLECTION_BY_AFFILIATE_USER_ID,
      type?.toString() || "all",
    ],
  ...
  return {
    affiliateCollections: data?.data,
    isLoading,
    error,
  };
};
