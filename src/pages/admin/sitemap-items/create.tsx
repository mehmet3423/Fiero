...
import { useCreateSitemapItem } from '@/hooks/services/sitemap-item/useCreateSitemapItem';
...
function CreateSitemapItemPage() {
  const router = useRouter();
  const { mutateAsync, isPending } = useCreateSitemapItem();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await mutateAsync({
        url: CREATE_SITEMAP_ITEM,
        method: HttpMethod.POST,
        data: { ...form, baseUrl },
      });
      if (response?.data?.isSucceed) {
        toast.success('Sitemap kaydı başarıyla oluşturuldu. Yönlendiriliyorsunuz...');
        setTimeout(() => {
          router.push('/admin/sitemap-items');
        }, 1200);
      } else {
        toast.error(response?.data?.message || 'Kayıt oluşturulamadı.');
      }
    } catch (err) {
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="d-flex gap-2 justify-content-end">
                  ...
                  <button type="submit" className="btn btn-primary" disabled={isPending}>
                    {isPending ? 'Kaydediliyor...' : 'Sitemap Kaydını Oluştur'}
                  </button>
                </div>
              </form>
            </div>
...
export default CreateSitemapItemPage;
