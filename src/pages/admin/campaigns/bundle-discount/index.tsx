...
const BundleDiscountPage = () => {
  ...
  const handleCreate = () => {
    router.push("/admin/campaigns/bundle-discount/create");
  };

  const handleEdit = (id: string) => {
    router.push(`/admin/campaigns/bundle-discount/edit/${id}`);
  };
  ...
  const bundleDiscounts = (discounts as unknown as BundleDiscount[]) || [];

  const getProductIds = (discount: BundleDiscount): string[] => {
          ...
          <table className="table">
            ...
            <tbody className="table-border-bottom-0">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="text-center">
                    Henüz bundle indirimi bulunmamaktadır.
                  </td>
                </tr>
              ) : (
                bundleDiscounts.map((discount) => (
                  <tr key={discount.id}>
                    ...
                    <td>
                      <span
                        className={`badge bg-${discount.isActive && discount.isWithinActiveDateRange
                          ? "success"
                          : "danger"
                          }`}
                      >
