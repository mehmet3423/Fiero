...
const CartDiscountPage = () => {
  ...
  const { discounts, isLoading, totalCount } = useGetDiscountList({
    ...
    isActive,
    discountType: DiscountType.Cart,
  });
  const { deleteDiscount, isPending: isDeleting } = useDeleteDiscount();
  const [deletingId, setDeletingId] = useState<string | null>(null);
  const handleCreate = () => {
    router.push("/admin/campaigns/cart-discount/create");
  };

  const handleEdit = (id: string) => {
    router.push(`/admin/campaigns/cart-discount/edit/${id}`);
  };
  const cartDiscounts = (discounts as unknown as CartDiscount[]) || [];
  ...
  const formatDiscountValue = (discount: CartDiscount) => {
    return discount.discountValueType === DiscountValueType.Percentage
          ...
          <table className="table">
            ...
            <tbody className="table-border-bottom-0">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="text-center">
                    Henüz sepet indirimi bulunmamaktadır.
                  </td>
                </tr>
              ) : (
                cartDiscounts.map((discount: CartDiscount) => (
                  <tr key={discount.id}>
                    <td>
                    ...
                    <td>{discount.cartDiscount?.minimumCartAmount} ₺</td>
...
export default CartDiscountPage;
