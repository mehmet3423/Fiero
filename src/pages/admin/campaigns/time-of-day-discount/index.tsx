...
const TimeOfDayDiscountPage = () => {
  ...
  // Cast the discounts to TimeOfDayDiscount[]
  const timeOfDayDiscounts = discounts as TimeOfDayDiscount[];
  ...
  const handleCreate = () => {
    router.push("/admin/campaigns/time-of-day-discount/create");
  };

  const handleEdit = (id: string) => {
    router.push(`/admin/campaigns/time-of-day-discount/edit/${id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success("Saat aralığı indirimi başarıyla silindi");
    } catch (error) {
      toast.error("Saat aralığı indirimi silinirken bir hata oluştu");
  ...
  const formatDiscountValue = (discount: TimeOfDayDiscount) => {
  ...
  const getTimeRangeBadgeColor = (startTime: number, endTime: number) => {
    if (
      startTime === null ||
      startTime === undefined ||
      endTime === null ||
    ...
    if (startTime >= 22 || endTime <= 6) return "bg-dark text-white";
    if (startTime >= 11 && endTime <= 14) return "bg-warning text-dark";
    if (startTime >= 17 && endTime <= 21) return "bg-info text-white";
    return "bg-primary";
  };
  ...
  return (
    <div className="container-fluid">
        ...
        <div className="table-responsive text-nowrap">
          <table className="table">
            ...
            <tbody className="table-border-bottom-0">
              ...
              ) : timeOfDayDiscounts?.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center">
                    Henüz saat aralığı indirimi bulunmuyor
                  </td>
                </tr>
              ) : (
                timeOfDayDiscounts?.map((discount: TimeOfDayDiscount) => {
                  // Parse time - handle both string format (HH:mm:ss) and ticks format
                  const parseTime = (time: any): number => {
                    ...
                    // If it's ticks format
                    if (time.ticks) {
                      return Math.floor(time.ticks / 36000000000);
                    }
                  ...
                  const safeStartTime =
                    parseTime(discount.timeOfDayDiscount?.startTime) || 9;
                  const safeEndTime =
                    parseTime(discount.timeOfDayDiscount?.endTime) || 17;
                  ...
                  return (
                    <tr key={discount.id}>
                      ...
                      <td>
                        ...
                        >
                          {formatTimeRange(safeStartTime, safeEndTime)}
                        </span>
                        {(discount.startTime === null ||
                          discount.startTime === undefined) && (
      ...
      <GeneralModal
        ...
        title="Saat Aralığı İndirimi Sil"
        size="sm"
      ...
      >
        <div className="text-center">
          ...
          <p className="text-muted">
            Bu saat aralığı indirimi silmek istediğinizden emin misiniz? Bu
            işlem geri alınamaz.
          </p>
        </div>
      </GeneralModal>
...
export default TimeOfDayDiscountPage;
